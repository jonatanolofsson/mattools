...
function generate(dirs, show_plots)
    ...
    if nargin < 2 || ~strcmpi(show_plots, 'On'), show_plots = 'off'; end;
    kvars.show_plots = show_plots;
    kvars.dirs = dirs;
    
    for notjustsomeanyf = 1:length(kvars.dirs)
        if strcmp(kvars.dirs{notjustsomeanyf}(1), '.'), continue; end;
        if exist(kvars.dirs{notjustsomeanyf}, 'dir')
            kvars.mdir = [kvars.dirs{notjustsomeanyf} '/matlab'];
            if exist(kvars.mdir, 'dir')
                kvars.mfile = [kvars.mdir '/' kvars.dirs{notjustsomeanyf} '.m'];
                if exist(kvars.mfile, 'file')
                    figures = Figures();
                    figures.plots_visible = kvars.show_plots;
                    disp(kvars.mfile);
                    run(kvars.mfile);
                    figures.save([kvars.dirs{notjustsomeanyf} '/figures']);
